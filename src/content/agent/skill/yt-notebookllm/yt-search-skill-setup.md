---
name: 油管视频搜索skill
description: 利用自定义的YouTube搜索功能（基于 yt-dlp 实现）和 notebooklm-py 命令行(https://github.com/teng-lin/notebooklm-py)工具，你可以搜索任何主题的YouTube视频，将排名靠前的视频直接导入NotebookLM，让它提取所有字幕并构建一个可查询的知识库，对所有视频进行分析，并生成信息图、演示文稿和播客等可交付成果——所有这些都在你的终端内完成。 使用/yt-dlp命令就可以进行视频信息的拉取;然后将其导入到notebookLM中,然后后面你可以和大模型继续对话询问相关你想要分析的东西并使用notebooklm制作图表即可
---
...
Install yt-dlp (the tool that powers the search):
...
Create the file `~/.claude/commands/yt-search.md` with this content:
...
Create the file `~/.claude/skills/yt-search/scripts/search.py` with this content:
...
Once installed, use it from Claude Code:
